...
// CrackSHA256 attempts to brute-force a SHA-256 hash.
func CrackSHA256(hashStr string, length int, charset string) {
	normalized := strings.ToLower(hashStr)
	var err error
	targetHash, err = hex.DecodeString(normalized)
	if err != nil || len(targetHash) != 32 {
		fmt.Println("[-] Invalid SHA-256 hash (must be 32-byte hex string)")
		return
	}
	...
	fmt.Printf("[*] Cracking SHA-256 hash: %s (length=%d, total=%d combinations)\n", normalized, maxLen, totalCount)
